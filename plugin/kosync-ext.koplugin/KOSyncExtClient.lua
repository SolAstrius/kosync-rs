...
local KOSyncExtClient = {
    service_spec = nil,
    custom_url = nil,
}

function KOSyncExtClient:new(o)
    if o == nil then o = {} end
    setmetatable(o, self)
    self.__index = self
...
function KOSyncExtClient:register(username, password)
    ...
    if ok then
        ...
        logger.dbg("KOSyncExtClient:register failure:", res)
        return false, res.body
    end
end

function KOSyncExtClient:authorize(username, password)
    self.client:reset_middlewares()
    self.client:enable("Format.JSON")
    self.client:enable("GinClient")
    ...
    if ok then
        ...
        logger.dbg("KOSyncExtClient:authorize failure:", res)
        return false, res.body
    end
end

function KOSyncExtClient:update_progress(username, password, document, progress, percentage, device, device_id, callback)
    self.client:reset_middlewares()
    self.client:enable("Format.JSON")
    self.client:enable("GinClient")
    self.client:enable("KOSyncAuth", {
    ...
    local co = coroutine.create(function()
        ...
        if ok then
            ...
            logger.dbg("KOSyncExtClient:update_progress failure:", res)
...
function KOSyncExtClient:get_progress(username, password, document, callback)
    self.client:reset_middlewares()
    self.client:enable("Format.JSON")
    self.client:enable("GinClient")
    self.client:enable("KOSyncAuth", {
    ...
    local co = coroutine.create(function()
        ...
        if ok then
            ...
            logger.dbg("KOSyncExtClient:get_progress failure:", res)
            callback(false, res.body)
        end
    end)
    self.client:enable("AsyncHTTP", {thread = co})
...
function KOSyncExtClient:get_annotations(username, password, document, callback)
    self.client:reset_middlewares()
    self.client:enable("Format.JSON")
    self.client:enable("GinClient")
    self.client:enable("KOSyncAuth", {
    ...
    local co = coroutine.create(function()
        ...
        if ok then
            ...
            logger.dbg("KOSyncExtClient:get_annotations failure:", res)
            callback(false, res.body)
        end
    end)
    self.client:enable("AsyncHTTP", {thread = co})
